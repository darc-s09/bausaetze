		...
		if (counter == 100)
			...
			for (uint8_t step = 0; step <= 9 ; step++) 	// Für Testzwecke werden die Daten als Binäre Zahl auf die LED`s 1 bis 10 geschrieben.
				...
				if(qbi(step,temperaturdaten))			
					{
					LED_TASK[step+1][0]=1;					// Zuschalten des Bits zur zugehörigen LED
					}
				else
					{
					LED_TASK[step+1][0]=0;					// Abschalten des Bits zur zugehörigen LED
					}
				}
...
über den Übergabeparameter wird festgelegt welche LED eingeschaltet wird
...
void led_multiplex(uint8_t ledcount)
{
	if(MAX_HELL < LED_HELLIGKEIT )
		{
			LED_HELLIGKEIT = MAX_HELL;  // hier wird die vorgegebene Helligkeit auf den maximal zulässigen Wert begrenzt
		}
		
	if (LED_Timer)
		{					// wenn LED Timer für Multiplexer 0, dann kann die nächste LED geschaltet werden
							// Diese Funktion steuert das Multiplexing und das Puls-/Pausenverhältnis der LED`s
		LED_Timer--;		// besser über Interrupt
		...
		{					// wenn LED Timer 0 wird die auf die nächste LED geschaltet 
...
TIMER2 > Für einen Interrupt aller XX ms
****************************************************************************************/
void TIMER_init(void)
{
	...
	/************** Timer 1 16 BIT Zähler **************************************/
	//TCCR1B = 0x05;	// Teiler 1024 3,68MHZ = 259ns
	//TCNT1H Zähler HIGH Byte (8bit)
...
Rücksetzbedingung in Bearbeitungsroutine: 
Controllword[0] 		> löschen
UART_PUFFER_VOLL,FLAGS 	> löschen
